 ...
 *   function(node, parent, property, index, depth) -> Boolean (false to stop)
...
function set_parent(node, parent, debug) {
    if (debug) {
        node.$parent = parent;
    } else {
        Object.defineProperty(node, "$parent", {enumerable: false, value: parent, writable: true});
    }
}
...
*/
function parentize(node, debug) {
    debug = debug || __debug;

    traverse(node, function(node, parent) {
        set_parent(node, parent, debug);
    });
}

var node_ids = 0;

function set_id(node, debug) {
    if (debug) {
        node.$id = ++node_ids;
    } else {
        Object.defineProperty(node, "$id", {enumerable: false, value: ++node_ids, writable: true});
    }
}
/**
* traverse AST and set an unique $id to every one
*/
...
function containmentFactor(anode, bnode) {
    ...
    for (i = 0; i< max; ++i) {
        ...
        for (j = 0; j < jmax; ++j) {
            if (contains(blocks[j], c)) {
                f = containmentFactor(blocks[j], c);
                if (f < min_f) {
                    min_f = f;
